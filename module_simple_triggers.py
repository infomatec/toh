...
#Party AI: pruning some of the prisoners in each center (once a week)
  ...
  #Checking if the empress was captured
  # (24,
   # [
	##--dirim quest
...
#--independant armies re-spawn
  (720,#every month
   [
     # (try_begin),
       # (store_num_parties_of_template, ":num_parties", "pt_black_army"),
       # (lt,":num_parties",1),
       # (store_random,":spawn_point",num_doomcall_spawn_points),
       # (val_add,":spawn_point","p_doomcall_spawn_point"),
       # (set_spawn_radius, 25),
       # (spawn_around_party,":spawn_point","pt_black_army"),
     # (try_end),	
     (try_begin),
       (store_num_parties_of_template, ":num_parties", "pt_desert_army"),
       (lt,":num_parties",1),
       (set_spawn_radius, 25),
       (spawn_around_party,"p_town_22","pt_desert_army"),
     (try_end),	
     (try_begin),
       (store_num_parties_of_template, ":num_parties", "pt_golden_horde"),
       (lt,":num_parties",1),
       (set_spawn_radius, 25),
       (spawn_around_party,"p_town_10","pt_golden_horde"),
     (try_end),
     (try_begin),
       (store_num_parties_of_template, ":num_parties", "pt_galnar_clan_army"),
       (lt,":num_parties",1),
       (set_spawn_radius, 25),
       (spawn_around_party,"p_town_2","pt_galnar_clan_army"),
     (try_end),	 	 
    ]),	
     ...
     [
        ...
        (try_begin),
          ...
          (neq, ":troop_no", ":faction_leader"),
			#--------------no more lord indictment or defection--------------
			(neg|is_between, ":troop_no", "trp_knight_1_1", "trp_knight_7_1"),
			#------------------------------------------------------------------
...
#    ]),
  ...
  # Only if the player is male -- female characters will be told that they should seek out a faction through NPCs, possibly
   ...
   # [
	 ...
	 # (try_end), 
    # ]),
	#-----------quand le joueur possede  un centre, il est invite a devenir vassal  --------
   (32,
   [
     (eq, "$players_kingdom", 0),
     (le, "$g_invite_faction", 0),
     ...
     (else_try),
	     ...
	     (try_begin),	       
	       (gt, ":num_centers_owned", 0),
		   (this_or_next|ge, "$g_player_fortress",2),
   ...
   [
     ...
     (try_begin),	       
       ...
       (ge, ":player_party_size", 10),
       #--no kingdom's quests
       (neg|check_quest_active,"qst_for_the_empire"),
       (neg|check_quest_active,"qst_long_life_to_the_king"),
       #--
       (enable_party,"p_ruins"),
      (else_try),  
   ...
   # (12,
   # [
     # (eq,"$g_player_fortress",1),
     # (eq, "$g_player_is_captive", 0),
	 # (jump_to_menu, "mnu_message_from_fortress"),
    # ]),
	#---------------------------------------------
...
#--------------------------------------------
  ...
  #remove freed nobles from player party
  (6,
   [
	(main_party_has_troop,"trp_imprisoned_nobleman"),
	(this_or_next|faction_slot_eq, "fac_player_supporters_faction", slot_faction_state, sfs_inactive),
	(lt, "$player_honor", 20),
	(party_count_companions_of_type, ":num_nobles", "p_main_party", "trp_imprisoned_nobleman"),
	(try_begin),
		(gt,":num_nobles",0),
		(party_remove_members, "p_main_party", "trp_imprisoned_nobleman", ":num_nobles"),	
		(assign, ":num_nobles", reg0),
		(try_begin),
			(gt, ":num_nobles", 1),
			(assign, reg2, 1),
		(else_try),
			(assign, reg2, 0),
		(try_end),
		(assign, reg1, ":num_nobles"),
		(str_store_troop_name_by_count, s1, "trp_imprisoned_nobleman", ":num_nobles"),
		(display_log_message, "@{reg1} {s1} {reg2?have:has} left your party and joined {reg2?his:their} homeland"),
		(store_mul, ":renown_gain", ":num_nobles", 2),
   ...
   [(assign, "$g_ransom_offer_rejected", 0),
    (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
      ...
      (try_for_range_backwards, ":i_stack", 0, ":num_stacks"),
        ...
        (store_random_in_range, ":random_no", 0, 100),
		#--Pravar never release the empress
		(store_troop_faction, ":faction_no", ":town_lord"),#deplace (voir plus bas)
		(try_begin),
			(neg|check_quest_concluded,"qst_lady_of_dirim_0"),
			(eq, ":stack_troop","trp_kingdom_7_lord"),
			(eq, ":faction_no" , "fac_kingdom_1"),
			(assign,":random_no", 90),#jamais relachee
		(try_end),
		#--
        (try_begin),
          (le, ":random_no", 10),
...
#    (ge, "$g_last_rest_center", 0),
  ...
  (36,
   [
	 ...
	 #-----------liberer les patrouilles du white order apres l'alerte-----------
		(try_for_parties,":party_id"),
		   ...
		   (party_set_ai_object, ":party_id", "p_new_order_camp"),
		   (party_set_ai_patrol_radius, ":party_id", 20),				
		(try_end),
	 #------------------------------------------------------------	   
    ]),
  ...
  (3, #check to see if player's court has been captured
   [
	  ...
	  #------------my castle pas de royaume --------
	  (try_begin), 
	   (gt, "$g_player_fortress", 2), # <--- le joueur n'a pas encore atteint l'etape creation de royaume
	   #---------------------------------------------
...
(1, 
   [
     ...
     #(assign, ":pravar_soldiers_are_upset_message_showed", 0),
     #(assign, ":borovod_soldiers_are_upset_message_showed", 0),
     #(assign, ":guhulay_soldiers_are_upset_message_showed", 0),
     #(assign, ":hadvog_soldiers_are_upset_message_showed", 0),
     #(assign, ":imifir_soldiers_are_upset_message_showed", 0),
...
#             (try_begin),
#               (eq, ":faction_no", "fac_kingdom_1"),  
#               (eq, ":pravar_soldiers_are_upset_message_showed", 0),
#               (str_store_string, s3, "str_pravar_soldiers_are_upset"),
#               (assign, ":pravar_soldiers_are_upset_message_showed", 1),
#             (else_try),
#               (eq, ":faction_no", "fac_kingdom_2"),  
#               (eq, ":borovod_soldiers_are_upset_message_showed", 0),
#               (str_store_string, s3, "str_borovod_soldiers_are_upset"),
#               (assign, ":borovod_soldiers_are_upset_message_showed", 1),
#             (else_try),
#               (eq, ":faction_no", "fac_kingdom_3"),  
#               (eq, ":guhulay_soldiers_are_upset_message_showed", 0),
#               (str_store_string, s3, "str_guhulay_soldiers_are_upset"),
#               (assign, ":guhulay_soldiers_are_upset_message_showed", 1),
#             (else_try),
#               (eq, ":faction_no", "fac_kingdom_4"),  
#               (eq, ":hadvog_soldiers_are_upset_message_showed", 0),
#               (str_store_string, s3, "str_hadvog_soldiers_are_upset"),
#               (assign, ":hadvog_soldiers_are_upset_message_showed", 1),
#             (else_try),
#               (eq, ":faction_no", "fac_kingdom_5"),  
#               (eq, ":imifir_soldiers_are_upset_message_showed", 0),
#               (str_store_string, s3, "str_imifir_soldiers_are_upset"),
#               (assign, ":imifir_soldiers_are_upset_message_showed", 1),
#             (try_end),
...
#           (try_end),
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  (24,
   ...
   #this to correct inappropriate home strings - Katrin to Lankladel, Matheld to Fearichen
   (troop_set_slot, "trp_npc11", slot_troop_home, "p_town_7"),
   (troop_set_slot, "trp_npc8", slot_troop_home, "p_village_35"),
   
   (troop_set_slot, "trp_npc15", slot_troop_town_with_contacts, "p_town_20"), #ukdud
   
   #this to correct linen production at villages of ukdud 
  ...
  (0,
   ...
   (try_begin),
		...
		(call_script, "script_night_restore_proficiencies"),
