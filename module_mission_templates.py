...
#--body armor
# def total_protection(item):
	# n = int(int(get_head_armor(item[6])) + int(get_body_armor(item[6])) + int(get_leg_armor(item[6])))
	# return n
# def head_protection(item):
	# n = int(get_head_armor(item[6]))
	# return n
# def body_protection(item):
	# n = int(get_body_armor(item[6]))
	# return n
# def leg_protection(item):
	# n = int(get_leg_armor(item[6]))
	# return n	
#--


# common_halberder_stand =(
  # 0, 1, ti_once, [],
   # [
      ##Run through all active NPCs on town.
      # (try_for_agents, ":agent"),
		# (agent_get_troop_id, ":agent_troop", ":agent"),
		# (eq,":agent_troop","trp_imifir_haberdier"),
		# (agent_set_wielded_item, ":agent", "itm_halberd"),
      # (try_end),   
   # ])
...
#--(Zarthas's [OSP] Camel Kit! - Makes horses react to camels +   xaliber code
horses_spook = (
	12, 0, 0, [],#Every 12(six) seconds
	[
	...
	[
	    ...
	    (try_for_agents, ":agent"),
			...
			(try_begin),
		    	...
		    	(agent_get_horse, ":horse", ":agent"),
		    	(eq, ":horse", -1), #No horse.
				#--toh 0.5 only spearmen or pikeman
                (agent_get_wielded_item, ":item", ":agent", 0), #Offhand.
				(gt, ":item", 0),
		    	(item_get_type, ":type", ":item"),
		    	(eq, ":type", itp_type_polearm), #polearm equipped.
				#--
				...
				(try_for_agents, ":suspect"),
					...
					(try_begin),
						...
						(display_message,"@joueur est proche!"),
					(try_end),
					(assign, ":minimum_distance", ":distance"),
					(assign, ":victim", ":suspect"),
				(try_end),
				...
				(try_begin),
					...
					(display_message,"@victime est le joueur!"),
				(try_end),
                ...
                (try_begin),#--shield bash/kicking
                    ...
                    (try_begin),
                        ...
                        (agent_set_animation, ":agent", "anim_shield_bash"),
                     (else_try),
                        (agent_set_animation, ":agent", "anim_kick_left_leg"),# anim_prepare_kick_0 exists as well
                    (try_end),
                    (agent_get_troop_id, ":troop", ":agent"),
                    (troop_get_type, ":type", ":troop"),
                    (try_begin),
                    ...
                    (else_try),
                        (agent_set_animation, ":victim", "anim_shield_strike"),
                    (try_end),	
				(else_try),#--run backward
                    (lt,":rand",6),
                    (agent_set_animation, ":agent", "anim_run_backward_onehanded"),
                (else_try),#--use backup weapon
					#(gt,":rand",1),
					(agent_get_troop_id, ":agent_troop", ":agent"),
					(troop_get_slot,":sidearm",":agent_troop",slot_troop_sidearm),
					(gt, ":sidearm", 0),
					(agent_get_wielded_item, ":item", ":agent", 0), 
					(agent_unequip_item, ":agent", ":item"),
					(agent_equip_item, ":agent", ":sidearm"),
					(agent_set_wielded_item, ":agent", ":sidearm"),
				(try_end),
				#--
			(try_end),
			(agent_get_slot, ":timer", ":agent", sp_agent_shield_bash_timer),
...
common_respawn_player = (
   ...
   [
		(eq, "$deathcam_on", 1),
		#--culcul healing time
		(party_get_skill_level, ":player_party_first_aid", "p_main_party", "skl_first_aid"),
		(gt,":player_party_first_aid",0),
        (store_div, ":time_to_heal",120,":player_party_first_aid"),#-- 2 mn / first_aid 
		#--
		(store_mission_timer_a,":cur_time"),
		(store_add,":rec_time","$wound_time",":time_to_heal"),# globl variable $wound_time recorded at the start of the death cam
...
#xenoargh's inspired damage system
#reg0 stores the item ID
#The returned value is the final damage output
...
[
	...
	# (try_begin),
		# (this_or_next|neq, ":attacker_troop", "trp_player"),
	...
	#--calculate body armor
	 # (agent_get_item_slot, ":head", ":agent", ek_head), 
     # (agent_get_item_slot, ":body", ":agent", ek_body), 
	# (agent_get_item_slot, ":leg", ":agent", ek_foot), 
	 # (try_begin),
		# (gt,":head",0),
		# (gt,":body",0),
		# (gt,":leg",0),
		# (item_slot_ge, ":head", slot_item_head_armor, 40),
		# (item_slot_ge, ":body", slot_item_body_armor, 50),
		# (item_slot_ge, ":leg", slot_item_leg_armor, 10),
		
		#(agent_is_human, ":agent"),#branch if not human
		#(agent_is_alive, ":agent"),
		# (agent_get_troop_id, reg19, ":agent"),
		# (store_agent_hit_points, reg20,":agent"),
		
		#(store_agent_hit_points, ":agent_hp",":agent"),
		# (is_between,reg20,1,10),
		# (display_message,"@{reg19} hp : {reg20}",dark_red),
		# (display_message,"@vulnerable+-+-+-"),	
		# (assign,reg0, "itm_mace_2"),
	...
	(try_begin),
		...
		(init_position,pos59),
		(agent_get_position,pos59,":agent"),
		#Do Blood simulations
		(try_begin),
			(is_between, ":damage", 1, 16),
				(position_move_z,pos59,125),
				(position_move_y,pos59,15),
				(particle_system_burst,"psys_blood_small",pos59,35),
		(else_try),	
			(is_between, ":damage", 15, 41),
				(position_move_z,pos59,125),
				(position_move_y,pos59,15),				
				(particle_system_burst,"psys_blood_small",pos59,35),
				(particle_system_burst,"psys_blood_medium",pos59,35),
		(else_try),	
			(gt, ":damage", 40),
				(position_move_z,pos59,125),
				(position_move_y,pos59,15),				
				(particle_system_burst,"psys_blood_small",pos59,35),
				(particle_system_burst,"psys_blood_medium",pos59,35),
				(particle_system_burst,"psys_blood_large",pos59,35),
		(try_end),	
	(try_end),
]
)

# common_heavy_armor_protection = (ti_on_agent_knocked_down, 0, 0, [],
# [
	# (store_trigger_param_1, ":agent"),
	##(store_trigger_param_2, ":attacker"),	
	# (agent_is_human, ":agent"),#branch if not human
...
common_force_lance =(0, 1, ti_once, [],
   [
      ...
      (try_for_agents, ":agent"),
		...
		(try_begin),
			...
			(try_for_range, ":i", ek_item_0, ":end"),
				...
				(this_or_next|is_between, ":weapontype", itp_type_one_handed_wpn, itp_type_polearm),#one or two handed
				(is_between, ":item", "itm_long_bardiche", "itm_shortened_military_scythe"),#or bardiche / hafted blade
				(assign,":has_choice",1),
			...
			(try_begin),# Equip their backup weapon.
				...
				(agent_set_wielded_item, ":agent", ":item"),
			(else_try),#--(toh 0.5)sidearm
				(troop_get_slot,":sidearm",":agent",slot_troop_sidearm),
				(gt,":sidearm",0),
				(agent_unequip_item,":agent", ":wielded"),
				(agent_set_wielded_item, ":agent", ":sidearm"),	
			(try_end),
			#-##-----caba drin adaptation end--------##-#
		(else_try),
			(agent_get_team, ":team", ":agent"),
			...
			(try_begin),
				...
				(lt,":random",50),#--50% chance qu'il change d'arme
				#-##-----caba drin adaptation--------##-#
				# Find non-lance/spear/bow item in inventory
				(assign,":has_choice",0),
				(assign, ":end", ek_head),
...
common_torches_at_night =(0, 1, ti_once, 
   ...
   [
	  ...
	  (try_for_agents, ":agent"),
        ...
        #(agent_get_horse, ":horse", ":agent"),
        #(eq, ":horse", 0),
        (agent_get_wielded_item, ":wielded", ":agent", 1),
		(gt, ":wielded", 0),
		(item_get_type, ":type", ":wielded"),
		(eq, ":type", itp_type_shield),
		#(display_message,"@bouclier!"),
        #(is_between, ":wielded", shields_begin, shields_end), 
		#(agent_set_wielded_item, ":agent", ":item"),
		#(store_random_in_range, ":rando", 0, 2),
        #(gt,":rando",1), #1 sur 3
        (agent_unequip_item, ":agent", ":wielded"),
		(agent_equip_item, ":agent", "itm_left_torch"),
		(agent_set_wielded_item, ":agent", "itm_left_torch"),
		(val_add,":comp",1),
      (try_end),   
   ])


   #-- maintaining  safe distance for ranged troops
common_skirmish_mod =(
   0.5, 0, 1, 
	[
	   (eq, "$ai_team_1_battle_tactic", 0),#--not "or" but "and" condition !
	   (eq, "$ai_team_2_battle_tactic", 0),
	],
   [
		(call_script,"script_skirmish_mod"),
   ])
   
   #-- cavalry movment
common_cavalry_movment =(
   0.5, 0, 1, 
	[
		(this_or_next|neq,"$ai_team_1_battle_tactic",0),
		(neq,"$ai_team_2_battle_tactic",0),
	],
   [
		(call_script,"script_cavalry_movment"),
   ])
   #--

#--(toh 0.51) bodysliding (part 2)
common_bodysliding = (5, 0, 0, 
	[
	...
	[
		...
		(try_for_agents, ":agent_no"),
			...
			(display_message, "@You have assumed control of {s31} to continue the battle.", cyan),
			### DIAGNOSTIC- ###
		(try_end),
...
#--(toh 0.51) bodysliding (part 2) END
## deathcam begin
common_init_deathcam = (
   0, 0, ti_once,
   [ ],
   [
        #(assign, "$deathcam_on", 0),#now with bodysliding ...
...
common_start_deathcam = (
   #-- 0, 1, ti_once, #1 second delay before the camera activates
    0, 1, 10, #1 second delay before the camera activates
    [
        (main_hero_fallen),
        (eq, "$deathcam_on", 0),
    ],
    [
        ...
        (display_message, "@You were wounded.", 0xFF2222),
		...
		#(store_mission_timer_a,"$wound_time"),
...
#--------bOuYA_k-hlO destructible doors-------------
common_destructible_door =(
5, 0, 0,
	[
		(store_mission_timer_a, ":time"),
		(gt, ":time", 60), # Si tu veux que ta porte ne soit detruite que apres 60 secondes
	],
	[
		(scene_prop_get_instance, ":instance", "spr_castle_f_door_a", 0),
		(scene_prop_get_hit_points, ":hp", ":instance"),
		(gt, ":hp", 0), 
		# Je ne suis pas sur de ce qui se passe si on fait des degats sur une porte detruite
		# Mais bon au moins ca evite de faire les calculs qui suivent quand la porte est detruite
		(get_player_agent_no, ":player_agent"),
		# On recupere le joueur car l'operation suivante necessite un agent pour faire les degats
		(prop_instance_receive_damage, ":instance", ":player_agent", 1337),
		# Si ta scene prop possede plus de points de vie que cette valeur, il faut l'augmenter
	])
#---------------------------------------------------
...
breakable_lances_trigger  = (
  ...
  [
   ...
   (try_begin),
      (gt, ":attacker", 0),
	  (neg|agent_is_non_player,":attacker"),
      #(agent_is_human, ":attacker"),
      (gt, ":damage", 20),

      (is_between, ":weapon", "itm_light_lance","itm_pike"),
               
      (agent_unequip_item,":attacker",":weapon"),

      # (agent_equip_item,":attacker","itm_staff"),
      # (agent_set_wielded_item, ":attacker", "itm_staff"),
   (try_end),
  ])
#-------------------------------------------

prisoner_cloths = [itm_shirt,itm_sword_medieval_a]
...
common_battle_mission_start = (
  ...
  [
	...
	# (try_begin),
		# (is_currently_night),
		# (display_message,"@is currently night time !!",red),
		##(prop_instance_get_position, pos2, banner_scene_props_begin),
		...
		# (spawn_scene_prop, "spr_fire_big"),#fire spawn
		# (spawn_scene_prop, "spr_light_night"),#fire spawn
	# (else_try),
		# (display_message,"@is currently day !!",green),
	# (try_end),
    #--(toh 0.5) body sliding part 1
    ...
    (assign, "$bodysliding_last_troop", "trp_player"),
	 #--(toh 0.5) body sliding part 1 END
    ])
...
common_battle_check_victory_condition = (
  ...
  [
    ...
    (finish_mission, 1),
	#-----manual loot----------------------
	#(troop_loot_troop, "trp_player", "trp_temp_troop_manual_loot", 100),
	#--------------------------------------
    ])

common_battle_victory_display = (
  10, 0, ti_once, [(eq,"$g_battle_won",1),],
  [
    (call_script, "script_cf_manual_loot"),
	(display_message,"str_msg_battle_won"), 
    ])
  ...
  (0, 0, ti_once, [],
   [
	 ...
	 (try_for_agents,":agent"),
		...
		(get_player_agent_no, ":player_agent"),
		(try_for_range, ":i", ek_item_0, ":end"),
			(agent_get_item_slot, ":item", ":player_agent",":i"),
			(gt, ":item", 0),
			(is_between, ":item", "itm_light_lance","itm_pike"),#
			(assign, ":end", ek_item_0),#loop breaker
			(assign,":continue",1),
		(try_end),
		(eq,":continue",1),
...
#-----------------------------------------------------------------
  ...
  (
    ...
    [(0,mtef_scene_source|mtef_team_0,af_override_horse,0,1,pilgrim_disguise),
     ...
     (9,mtef_visitor_source,af_override_horse,0,1,[]),(10,mtef_visitor_source,af_override_horse,0,1,[]),(11,mtef_visitor_source,af_override_horse,0,1,[]),(12,mtef_visitor_source,af_override_horse,0,1,[]),

	 (13,mtef_visitor_source,0,0,1,[]),
	 #(13,mtef_visitor_source,af_override_all,0,1,[]),#--------attention !!! tout nu
	 #------darenbay secret prison--------
	 #(14,mtef_scene_source,0,0,1,[]),
	 (14,mtef_scene_source|mtef_team_0,af_override_horse,0,1,pilgrim_disguise), 
	 #-----------------------------------
	 (15,mtef_scene_source,0,0,1,[]),
    ...
    (1, 0, ti_once, 
	  ...
	  [
		#(display_message,"@alert dans la ville"),
		(get_player_agent_no, ":player_agent"),
   ...
   [
     ...
     (try_begin), 
       (this_or_next|eq, ":dead_agent_troop_no", "trp_pravar_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_borovod_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_guhulay_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_hadvog_prison_guard"),
       (this_or_next|eq, ":dead_agent_troop_no", "trp_imifir_prison_guard"),
       (eq, ":dead_agent_troop_no", "trp_afirid_prison_guard"),
...
#--------------------------darenbay secret prison--------------- 
  (
    "darenbay_secret_prison",mtf_battle_mode,-1,
    "You enter the halls of the lord.",
	[(0,mtef_scene_source|mtef_team_0,af_override_horse,aif_start_alarmed,1,[]),
	 ...
	 (2,mtef_visitor_source|mtef_team_1,af_override_head,aif_start_alarmed,1,[]),
	 (3,mtef_visitor_source|mtef_team_1,af_override_head,aif_start_alarmed,1,[]),
     ],
    [
      # (ti_on_leave_area, 0, 0,
      # [
      ...
      (0, 0, ti_once, 
       ...
       [
		 ...
		 (mission_enable_talk),
		 (add_visitors_to_current_scene, 1, "trp_npc17", 1, 0),   
		(start_mission_conversation,"trp_npc17"),		 
       ]),
    ],
  ),
 #------------------------------------
  ...
  (
    ...
    [(0,mtef_team_0|mtef_use_exact_number,af_override_horse|af_override_weapons|af_override_head,aif_start_alarmed,4,[itm_boar_spear]),
     (3,mtef_visitor_source|mtef_team_1,af_override_horse,aif_start_alarmed,1,[]),
     ],
    [
      common_inventory_not_available,
	...
	## deathcam end
      ...
      #--(toh 0.51) bodysliding END
       ...
       [
		 ...
		 (agent_is_non_player, ":agent_no"), #<-- for the respawn player script
		 ...
		 #--agent protection
		# (agent_get_item_slot, ":head", ":agent_no", ek_head), 
		# (agent_get_item_slot, ":body", ":agent_no", ek_body), 
		# (agent_get_item_slot, ":leg", ":agent_no", ek_foot), 
		# (try_begin),
			# (gt,":head",0),
			# (gt,":body",0),
			# (gt,":leg",0),
       ...
       [
        ...
        (try_begin),
          ...
          (agent_is_human, ":dead_agent_no"),
			#--agent protection
			(assign,":armoured",0),
			(agent_get_item_slot, ":head", ":dead_agent_no", ek_head), 
			(agent_get_item_slot, ":body", ":dead_agent_no", ek_body), 
			(agent_get_item_slot, ":leg", ":dead_agent_no", ek_foot), 
			(try_begin),
				(gt,":head",0),
				(gt,":body",0),
				(gt,":leg",0),
...
##          (agent_get_team, reg3, ":dead_agent_no"),          
		...
		(end_try),
		(try_begin),#--wound armoured troops
			(store_random_in_range, ":random", 1, 10),
			(lt,":random",8),
			#(eq, ":is_wounded", 0),
			(eq,":armoured",1),
			(agent_is_non_player, ":dead_agent_no"), #<-- for the respawn player script
			(agent_get_party_id,":agent_party",":dead_agent_no"),
			(eq,":agent_party","p_main_party"), 
			(party_remove_members, "p_main_party", ":dead_agent_troop_id", 1),
			(party_add_members, "p_main_party", ":dead_agent_troop_id", 1),
      ...
      (ti_before_mission_start, 0, 0, [],
       [
		 ...
		 #--init teams values (cavalry_movment)
		 # (assign,"$team1",-1),
		 # (assign,"$team2",-1),
		 (assign,"$team1_x",-1),
		(assign,"$team1_y",-1),
		(assign,"$team2_x",-1),	 
		(assign,"$team2_y",-1),	 
		(assign,"$player_team_x",-1),	 
		(assign,"$player_team_y",-1),
		 #--
      ...
      (lt,"$defender_reinforcement_stage","$g_defender_reinforcement_limit"),
		   ...
		   #--respawn player
		   # (try_begin),
				# (display_message,"@respawn def",dark_blue),
				# (set_spawn_position, pos63),
				# (spawn_scene_prop, "spr_grave_a"),
				# (eq, "$deathcam_on", 1),
				  # (get_player_agent_no, ":player_agent"),
				  # (agent_get_position, pos3, ":player_agent"),
				  # (set_spawn_position, pos3),
				  ...
				  # (agent_set_team, ":player_agent", ":player_team"),
				  # (agent_set_hit_points,":player_agent",5),				  
				  # (display_message,"@player spawn"),
				
				##(get_player_agent_no, ":player_agent"),    
				##(agent_get_entry_no, ":player_entry", ":player_agent"),
				##(add_visitors_to_current_scene, ":player_entry", "$g_player_troop", 1),
		   # (try_end),
		   #--
		   ]),
      ...
      (1, 0, 5, [(lt,"$attacker_reinforcement_stage",2),
		   ...
		   #--respawn player
		   # (try_begin),
				# (display_message,"@respawn attackers",dark_red),
				# (set_spawn_position, pos63),
				# (spawn_scene_prop, "spr_grave_a"),
	...
	(0, 0, 0, [(key_clicked, key_n),
		...
		(get_player_agent_no, ":player_agent"),
		(agent_get_team,":player_team", ":player_agent"),
		(assign,reg1,":player_team"),
		(display_message,"@ player team : {reg1}",green),
		(assign, ":continue",0),
		(try_for_agents,":agent"),
			(eq, ":continue",0),
			(neq,":agent",":player_agent"),
			...
			(eq,":agent_team","$ai_team_1"),
			(assign,reg2,":agent_team"),
			(display_message,"@ team 1 : {reg2}",red),
			(assign, ":continue",1),
		(try_end),
		(try_for_agents,":agent"),
			(eq, ":continue",1),
			...
			(agent_get_team,":agent_team", ":agent"),
			(neq,":agent_team",":player_team"),
			(eq,":agent_team","$ai_team_2"),
			(assign,reg3,":agent_team"),
			(display_message,"@ team 2 : {reg3}",blue),
			(assign, ":continue",0),
		(try_end),		
...
##  ),
  ...
  (
	...
	## deathcam end
      ...
      (0, 0, ti_once, [], [(assign,"$g_battle_won",0),
                           (call_script, "script_music_set_situation_with_culture", mtf_sit_ambushed),
                           #--(toh 0.5) pravar quest - siege of dirim - inner battle
						   (try_begin),
...
(1, 4, ti_once,
  ...
  #-- pravar quest (dirim siege - camp's meeting)
  (
    "siege_camp_meeting",0,-1,
    "Meeting a Pravaran lord",
    [
    ...
    [
      #(ti_before_mission_start, 0, 0, [], [(call_script, "script_change_banners_and_chest")]),
		...
		[
			...
			(start_mission_conversation, ":giver_troop"),
		]),
  ...
  #-- pravar quest (dirim siege - alley battle)
  (
    "besiege_inner_battle_town_alley",0,-1,
    "You attack the back alley...",
    [
     (0,mtef_scene_source|mtef_team_0,af_override_horse,0,1,[]),
	...
	(1, 0, ti_once,
		...
		[
			...
			(store_current_scene, ":cur_scene"),
			(modify_visitors_at_site, ":cur_scene"),
			(try_for_range, ":slot_no", 1, ":num_stacks"),
				...
				(gt,":level",20),
				(neg|is_between,":troop_no","trp_common_prisoner","trp_veteran_pikeman"),
				(add_visitors_to_current_scene, 1, ":troop_no", 1),
			(try_end),
			(add_visitors_to_current_scene, 1, "trp_pravar_man_at_arms", 1),
			(add_visitors_to_current_scene, 1, "trp_pravar_infantry", 1),
			(add_visitors_to_current_scene, 1, "trp_pravar_knight", 1),
			(team_give_order, 0, grc_everyone, mordr_charge),
		]),	
		...
		[
			...
			(try_for_agents, ":cur_agent"), 
					...
					(eq,":troop_type","trp_finjir"),
					(agent_get_position, pos0, ":cur_agent"),
					(assign, ":break", 1),
			(try_end),
			(agent_is_in_line_of_sight, ":player_agent", pos0),
		],
		[
			(start_mission_conversation, "trp_finjir"),
			(mission_disable_talk),
			# (try_for_agents, ":cur_agent"), 
				# (agent_get_troop_id, ":troop_type", ":cur_agent"),
				# (this_or_next|eq,":troop_type","trp_finjir"),
				# (is_between, ":troop_type", "trp_dirim_recruit", "trp_company_colonial_militia"),
				# (agent_set_team, ":cur_agent", 1),
			# (try_end),
			#(display_message,"@Your men are coming to your Rescue, We must resist until they comes"),
			(troop_set_slot, "trp_finjir", slot_troop_met_previously, 1),
			(set_party_battle_mode),
	...
	(1, 4, ti_once,
			...
			[
				(finish_mission,0),
				(jump_to_menu,"mnu_player_permanent_death"),
			]),
    ...
    [
      ...
      (0, 0, 0, [(try_begin),
                 ...
                 (else_try),
                   ...
                   (reset_visitors),
                   (set_visitor,5,"trp_borovod_trained_archer"),
                   (set_visitor,6,"trp_borovod_trained_archer"),
                   (set_visitor,7,"trp_borovod_trained_archer"),
       ...
       [
         ...
         (try_for_range, ":player_no", 0, ":num_players"),
           ...
           (try_begin),
             ...
             (this_or_next|eq, ":item_id", "itm_warhorse_afirid"),
	   ...
	   [
        ...
        (try_for_agents, ":agent_no"),
          ...
          #(assign, "$g_talk_troop", "trp_relative_of_merchant"),
   		...
   		(troop_set_slot,"trp_npc11", slot_troop_met_previously, 1),
...
#           (check_quest_active, "qst_save_relative_of_merchant"),
  ...
  (
    ...
    [
	#(2,mtef_visitor_source|mtef_team_1,af_override_horse, aif_start_alarmed,1,[]),
	  (0,mtef_scene_source|mtef_team_0,af_override_all,0,1,[]),
      (1,mtef_scene_source|mtef_team_0,af_override_horse,0,1,[]),
      ...
      (5,mtef_visitor_source|mtef_team_0,af_override_horse,0,1,[]),
      (6,mtef_visitor_source|mtef_team_0,af_override_horse,0,1,[]),
      (7,mtef_visitor_source|mtef_team_0,af_override_all,0,1,prisoner_cloths),          
	  (8,mtef_visitor_source|mtef_team_0,af_override_all, aif_start_alarmed,8,prisoner_cloths),	  
	  (9,mtef_visitor_source|mtef_team_1,af_override_horse,aif_start_alarmed,4,[]),

	  # (24,mtef_visitor_source|mtef_team_0,af_override_horse, aif_start_alarmed,5,[]),
	  # (40,mtef_visitor_source|mtef_team_1,af_override_horse, aif_start_alarmed,1,[]),
	  # (41,mtef_visitor_source|mtef_team_1,af_override_horse, aif_start_alarmed,1,[]),
	  # (42,mtef_visitor_source|mtef_team_1,af_override_horse, aif_start_alarmed,1,[]),
	  # (43,mtef_visitor_source|mtef_team_1,af_override_horse, aif_start_alarmed,1,[]),
      ...
      (1, 4, ti_once,
       ...
       [
         (try_begin),
           ...
           (jump_to_menu, "mnu_retake_castle_failed"),
         (else_try),
           (jump_to_menu, "mnu_retake_castle_succeeded"),
         (try_end),
		 (finish_mission),
         ]),
      ]),  
...
##		]),
		...
		[
		   (assign,":sneak_distance",2000),
			...
			(try_for_agents, ":cur_agent"),
				(agent_get_troop_id, ":cur_agent_troop", ":cur_agent"),
				(eq, ":cur_agent_troop", "trp_order_priest"),
				(assign,"$priest_1",":cur_agent"),
				(agent_get_position,pos2,":cur_agent"),
				(get_distance_between_positions,":distance",pos1,pos2),
     ...
     (2, 0, ti_once, [(main_hero_fallen),],
	 [
		   #(display_message, "@ You has been bested  by a dark priest...Press Tab to exit"),
			(finish_mission),        
			(leave_encounter),
			(change_screen_return),
     ]),		 
...
###para items si esta vivo
      (2, 0, ti_once, [ (neg|agent_is_alive,"$priest_1"),], 
	   [
			(display_message, "@ You have beaten the dark priest ..."),
            #(troop_add_item, "trp_player", "itm_coat_of_plates",0),
            (call_script, "script_troop_add_gold", "trp_player", 1000),
			(call_script, "script_change_troop_renown", "trp_player", 20),
            (assign,"$g_dark_priest",1),
			(assign,"$g_artisan_dialog_end",0),
			#(display_message, "@Press TAB to leave"),
       ]),
...
###chief acaba                       
	...
	common_battle_victory_display,
		...
		[
		   (assign,":sneak_distance",1000),
			...
			(try_for_agents, ":cur_agent"),
				(agent_get_troop_id, ":cur_agent_troop", ":cur_agent"),
				(eq, ":cur_agent_troop", "trp_order_gunner"),
				(agent_get_position,pos2,":cur_agent"),
				(get_distance_between_positions,":distance",pos1,pos2),
				(lt,":distance",":sneak_distance"),
				(assign,":continue",1),
		...
		[
			...
			(start_mission_conversation, "trp_order_gunner"),

		]),
        (0.1, 0, ti_once, 
		[
		   (troop_slot_eq, "trp_pedran", slot_troop_met_previously, 0),
		   (assign,":sneak_distance",1000),
			(get_player_agent_no,":player_agent"),
			(agent_get_position,pos1,":player_agent"),
			(assign,":continue",0),
		...
		[
			(assign, "$talk_context", tc_camp_talk),
			(start_mission_conversation, "trp_order_billman"),
		]),
        (0.1, 0, ti_once, 
		[
		   (troop_slot_eq, "trp_pedran", slot_troop_met_previously, 0),
		...
		[
			...
			(try_for_agents, ":cur_agent"), 
				(agent_get_troop_id, ":troop_type", ":cur_agent"),
				(this_or_next|eq,":troop_type","trp_pedran"),
				(is_between, ":troop_type", "trp_order_billman", "trp_black_guhulay_horseman"),
				(agent_set_team, ":cur_agent", 1),
			(try_end),
			(troop_set_slot, "trp_pedran", slot_troop_met_previously, 1),
		]),	
		...
		(1, 4, ti_once,
			...
			[
				(display_message, "@You were defeated! Press TAB to end the battle."),
				#(assign, "$pin_player_fallen", 1),
				(finish_mission, 0),
				(jump_to_menu, "mnu_new_order_camp_defeat"),
			]),
		(1, 4, ti_once,
			[
				(assign, ":eteam_alive", 0),
				(try_for_agents, ":agent"), #Check enemy team is dead
					(neq, ":eteam_alive", 1), #Break loop
					(agent_get_troop_id, ":agent_troop_id", ":agent"),
					(this_or_next|eq,":agent_troop_id","trp_pedran"),
					(is_between, ":agent_troop_id", "trp_order_billman", "trp_black_guhulay_horseman"),
					(agent_is_alive, ":agent"),
					(assign, ":eteam_alive", 1),
				(try_end),
				(eq, ":eteam_alive", 0),
			],
			[
				(mission_enable_talk),
				(display_message, "@Battle won!"),
				...
				(try_begin),
				  ...
				  (assign, ":player_agent", reg0),
				  (display_message,"@player spawn"),
				  #(agent_set_hit_points, ":player_agent", 10),
					#----------------------
				(else_try),
					(start_mission_conversation, "trp_charob"),
				(try_end),
				#(jump_to_menu, "mnu_new_order_camp_victory"),
			]),	
			
		(1, 4, ti_once,
			[
				(troop_slot_eq, "trp_pedran", slot_troop_met_previously, 1),
			],
			[
				(party_get_num_companion_stacks, ":num_stacks", "p_main_party"),
				(store_current_scene, ":cur_scene"),
				...
				(try_for_range, ":slot_no", 1, ":num_stacks"),
					...
					(try_begin),
						...
						(val_sub,":stack_size", ":num_wounded"),
					(try_end),
					(add_visitors_to_current_scene, 10, ":troop_no", ":stack_size"),
				(try_end),
			]),	
  ...
  #-------------------------------------------------------      
 #---------------------------white order camp assault-----------------
  (
    "new_order_camp_assault",mtf_battle_mode|mtf_synch_inventory,-1,
    "You attack your enemies camp...",
    [(0,mtef_scene_source|mtef_team_0,af_override_horse,aif_start_alarmed,1,[]),
  ...
  (
    ...
    [
      ...
      [              
        ...
        (try_begin),
          (agent_get_troop_id, ":troop_no", ":agent_no"),
          (is_between, ":troop_no", "trp_pravar_merchant", "trp_startup_merchants_end"),
          (agent_set_team, ":agent_no", 7),          
          (team_set_relation, 0, 7, 0), 
        (try_end),                
      ]),
        ...
        (lt, "$merchant_sign_count", 8),
          ...
          (get_player_agent_no, ":player_agent"),
  	      (try_for_agents, ":agent_no"),
  	        ...
  	        (ge, ":agent_troop_id", "trp_pravar_merchant"),
